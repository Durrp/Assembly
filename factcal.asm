...
userin:		
		...
		; Save factorial on to stack
		push ax
		
		;Display factorial message
		mov dx, OFFSET factmsg
		mov ah, 09h
		int 21h
...
;Recursive procedure to calculate a factorial number 1-8
refact PROC
		;pop the offset into bx
		pop bx
		;move the number to be calculated
		pop ax
		;move the offset back onto the stack
		push bx	
		...
		; caclulate the factorial value n*(n-1)
		pop bx
		mul bx
...
;Procedure used to display a number in base 10
disp10 PROC
		;pop the offset into bx
		pop bx
